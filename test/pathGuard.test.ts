...
import { ensureValidHeapPath } from "../src/security/pathGuard.js";

function setup(): { root: string; heapFile: string } {
  ...
  return { root, heapFile };
}

test("ensureValidHeapPath accepts a valid heap file", () => {
  const { heapFile } = setup();
  const resolved = ensureValidHeapPath(heapFile);
